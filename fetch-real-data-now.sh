...
echo "🇮🇳 =================================================="
echo "   Fetching Real MGNREGA Data from API"
echo "   =================================================="
echo ""
...
CURRENT_DATA=$(curl -s http://localhost:5001/api/api-status)
echo "Current Status: $CURRENT_DATA"
echo ""

echo "🔑 Step 2: Checking API key..."
API_KEY=$(grep MGNREGA_API_KEY .env | cut -d '=' -f2)

if [ "$API_KEY" == "your_api_key_here" ] || [ -z "$API_KEY" ]; then
    echo "❌ No valid API key found!"
    echo ""
    echo "📝 To get a real API key:"
    echo "   1. Visit: https://data.gov.in/user/register"
    echo "   2. Sign up and verify email"
    echo "   3. Login and go to: My Account → API Keys"
    echo "   4. Generate API key"
    echo "   5. Add to .env: MGNREGA_API_KEY=your_key"
    echo ""
    echo "For now, I'll create enhanced mock data with realistic values..."
    echo ""
else
...
echo "🗑️ Step 3: Clearing old data..."
mongosh mongodb://127.0.0.1:27017/mgnrega --eval "db.districts.deleteMany({})" --quiet
echo "✅ Database cleared"
echo ""

echo "📡 Step 4: Creating enhanced data fetcher..."

# ============================================
# ENHANCED REAL DATA FETCHER
# ============================================
cat > scripts/fetchRealDataNow.js << 'EOF'
...
echo "✅ Enhanced fetcher created"
echo ""

echo "🚀 Step 5: Running enhanced data fetch..."
node scripts/fetchRealDataNow.js
...
echo ""
echo "📋 States:"
curl -s http://localhost:5001/api/states | jq '.success, .count, (.data | .[0:3])'

echo ""
echo "🏛️ Sample District (Bihar - Patna):"
curl -s http://localhost:5001/api/districts/04001 | jq '.success, .data.name, .data.state, .data.dataSource, (.data.monthlyMetrics | length)'
...
echo "   • 200+ districts across 20 states"
echo "   • 6 months of realistic data per district"
echo "   • Seasonal variations in employment"
echo "   • Metro/city/rural classifications"
echo ""
echo "🎯 Next Steps:"
echo "   1. Restart backend: npm run dev"
echo "   2. Restart frontend: cd ../frontend && npm start"
echo "   3. Open: http://localhost:3000"
echo "   4. Select any state and district to see data"
echo ""
echo "🔑 To get REAL API data:"
echo "   1. Get API key from https://data.gov.in"
echo "   2. Add to .env: MGNREGA_API_KEY=your_key"
echo "   3. Run: npm run fetch"
echo ""
